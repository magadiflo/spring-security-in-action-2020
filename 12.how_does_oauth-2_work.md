...
# [Pág. 299] Implementación de una aplicación simple de inicio de sesión único
...
Una aplicación de inicio de sesión único (single sign-on, SSO) es, como sugiere el nombre, una en la que se autentica a
través de un servidor de autorización y, luego, la aplicación lo mantiene conectado mediante un token de actualización.
En nuestro caso, representa solo al cliente de la arquitectura OAuth 2.
...
A continuación se muestra el código completo de nuestra clase de configuración de Spring Security, esto incluye la
exposición del @Bean ClientRegistrationRepository:
